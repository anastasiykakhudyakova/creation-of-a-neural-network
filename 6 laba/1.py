...
#Функции активации вводят нелинейность. Т.е. скрытый слой рассчитывается уже не обычным перемножением входного значения на вес выходного, но и дополнительно 
"""умножается на спец функцию активации.Одна из самых популярных подобных функций это функция ReLU – Rectifiet Linear Unit"""
...
"""Скрытые слои - это промежуточные слои нейронной сети между входным и выходным слоями.
Они называются "скрытыми", потому что мы не видим их работу напрямую - только входные и выходные данные."""

#Свойства ReLU:
#Устранение проблемы затухающих градиентов: производная равна 1 для положительных значений
#отрицательные значения обнуляются, что способствует разреженным представлениям
...
"""Добавление еще одного скрытого слоя кардинально меняет архитектуру сети - мы переходим от широкой сети к глубокой.
Один слой:
Извлекает признаки первого порядка из сырых данных
Два слоя:
Первый слой: Извлекает простые признаки (как и раньше)
Второй слой: Комбинирует простые признаки в сложные признаки"""
